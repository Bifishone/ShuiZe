...
class ColorPrinter:
    ...
    @classmethod
    def info(cls, msg):
        print(f"{cls.OKBLUE}[*] {msg}{cls.ENDC}")
    ...
    @classmethod
    def warning(cls, msg):
        print(f"{cls.WARNING}[!] {msg}{cls.ENDC}")
                ...
                if keyword in each:
                    ...
                    if githubAddr:
                        ...
                        try:
                            ...
                            ColorPrinter.highlight(f'github address: [line:{githubAddr}] {raw_url}')
                            ColorPrinter.info(f'[emails] : {emails}')
                            ColorPrinter.highlight(f'[{keyword}] [line:{line}] {content[line - 1].strip()}')
                            ColorPrinter.info(f'[{keyword}] [line:{line + 1}] {content[line].strip()}')
                            ColorPrinter.highlight(f'[{keyword}] [line:{line + 2}] {content[line + 1].strip()}')
...
def printGetNewSubdomains(old_subdomains, new_subdomains):
    if len(old_subdomains) > 0:
        ...
        ColorPrinter.info(f'[new :{len(newSubdomains)}] {newSubdomains}')
...
def checkPanAnalysis():
    ...
    panDomain = 'sadfsadnxzjlkcxjvlkasdfasdf.{}'.format(domain)  # 移除http://前缀，避免DNS解析错误
    panUrl = 'http://{}'.format(panDomain)  # 单独作为URL使用

    def checkPanAnalysis1(panDomain):
        try:
            # 手动指定DNS服务器（避免依赖系统配置）
            resolver = dns.resolver.Resolver()
            resolver.nameservers = ['8.8.8.8', '114.114.114.114']  # 公共DNS服务器

            answers = resolver.resolve(panDomain, 'A', raise_on_no_answer=False)
            if not answers:
                return False
            dns_A_ips = [str(j) for j in answers]
            cprint(f"DNS解析结果: {dns_A_ips}", 'cyan')
            ...
            return False
        # 捕获NoneType导致的AttributeError
        except AttributeError as e:
            print(f"DNS配置错误: {str(e)}，已使用备用DNS服务器")
            return False
        except Exception as e:
            print(f"意外错误(DNS): {str(e)}")
            return False

    def checkPanAnalysis2(panUrl):
        try:
            # 延长超时时间到10秒，增加连接超时设置
            res = requests.head(
                panUrl,
            ...
            print(f"HTTP请求: {panUrl} 超时")
            return False
        except requests.ConnectionError:
            print(f"HTTP请求: {panUrl} 连接失败")
            return False
        except Exception as e:
    ...
    try:
        # 先进行DNS检查，再进行HTTP检查
        dns_result = checkPanAnalysis1(panDomain)
        http_result = checkPanAnalysis2(panUrl)

        if dns_result or http_result:
            print(f'[泛解析] {panDomain}')
            return True
        else:
            print('[不是泛解析]')
            return False
    except Exception as e:
        print(f"泛解析检查整体出错: {str(e)}")
        # 如果仍然出错，强制返回不是泛解析
        print('[不是泛解析]')
        return False


# 如果上述修改仍有问题，可直接注释掉该功能的调用处
# 并将函数改为直接返回False，彻底禁用该功能：
def checkPanAnalysis():
    cprint('-' * 50 + 'check Pan-Analysis (临时禁用) ...' + '-' * 50, 'green')
    print('[不是泛解析]')
    return False
...
# 调用Sublist3r子域名收集脚本
def Sublist3r():
    ColorPrinter.success('Load Sublist3r Subdomain ...')
...
def get_ip_address(web_ip_list):
    cprint('-' * 50 + 'IP Address Query' + '-' * 50, 'cyan')  # 对IP进行归属地查询
...
def scan_IntranetPorts():
    ...
    tqdm.write(Fore.YELLOW + '-' * 50 + 'scan_IntranetPorts ...' + '-' * 50)
    web_host_port, service_host_port, alive_host_List = scanPort.run_ScanPort(allTargets_Queue, proxy)

    tqdm.write(Fore.YELLOW + '-' * 50 + 'get_IntranetHostName and IP ...' + '-' * 50)
...
def run_getWebTitle(web_host_port, ip_address_dict):
    tqdm.write(Fore.GREEN + '-' * 50 + 'run_getWebTitle ...' + '-' * 50)  # 筛选存活并获取标题
...
def detect_webVul(alive_Web):
    ...
    def runSelfVul():
        ...
        for vulName in vulList:
            tqdm.write(Fore.YELLOW + '-' * 50 + 'detect ' + vulName[:-3] + '-' * 50)  # 探测各种漏洞
            ...
            try:
                if hasattr(md, 'Detect'):
                    ...
                    if isIntranet == 1:
                        ...
                        if vulName in intPassVul:
                            ...
                            tqdm.write(Fore.YELLOW + '内网不跑{}漏洞'.format(vulName))
                            continue
                    else:
                ...
                tqdm.write(Fore.YELLOW + r'[-] Load Vul [{}] Error: {}'.format(vulName, e.args))
                continue
    ...
    tqdm.write(Fore.YELLOW + '-' * 50 + 'detect Web vul' + '-' * 50)  # 探测各种漏洞
...
def detect_paramVul(param_Links):
    tqdm.write(Fore.YELLOW + '-' * 50 + 'detect param vul' + '-' * 50)  # 探测各种参数漏洞-注入
...
def detect_unauthWeakVul(service_host_port):
    tqdm.write(Fore.YELLOW + '-' * 50 + 'detect unauth vul' + '-' * 50)  # 探测各种漏洞
    tqdm.write(Fore.YELLOW + 'service_host_port : {}'.format(service_host_port))
    ...
    def detect_unauthWeak(protocol, ip, port):
        if protocol in serviceVulName.keys():
            ...
            tqdm.write(Fore.YELLOW + 'test [{}] : {} {}'.format(vulName, ip, port))
...
def detect_winVul(alive_host_List):
    ...
    for vulName in vulList:
        ...
        try:
            ...
            ColorPrinter.error(f'Load Vul [{vulName}] Error: {e.args}')
            continue
...
def printSave_Vul(Vul_list):
    if Vul_list:  # 如果探测出漏洞
        tqdm.write(Fore.YELLOW + '-' * 50 + 'Vulnerabilities exist ' + '-' * 50)  # 探测各种漏洞
        for vul in Vul_list:
            Vul_Name, Vul_url, Vul_exist = vul
            tqdm.write(Fore.RED + '[{}] {} {}'.format(Vul_Name, Vul_url, Vul_exist))
        ...
        tqdm.write(Fore.YELLOW + '-' * 50 + 'Non-existent vulnerabilities' + '-' * 50)
...
def run_subdomain():
    ...
    # ColorPrinter.info(f'Subdomains_ips: {Subdomains_ips}')
    ...
    ColorPrinter.info(f'[total: {len(ksubdomains)}] ksubdomain: {ksubdomains}')
    subdomains = printGetNewSubdomains([], ksubdomains)
    ColorPrinter.highlight(f'len [{len(subdomains)}]')

    # 1. theHarvester
    theHarvesterSubdomains = []  # theHarvester()
    ColorPrinter.info(f'[total: {len(theHarvesterSubdomains)}] theHarvester: {theHarvesterSubdomains}')
    ...
    ColorPrinter.highlight(f'[total: {len(githubApiSubdomains)}] Github: {githubApiSubdomains}')
...
'''
    ...
    CIP_List = []
    ColorPrinter.highlight(f'C段的IP: {CIP_List}')
...
def run_cSubnet(CIP_List, Subdomains_ips, notCDNSubdomains, param_Links):
    print(CIP_List)
    ColorPrinter.info(f'Subdomains_ips: {Subdomains_ips}')
    ColorPrinter.highlight(f'notCDNSubdomains: {notCDNSubdomains}')

    SubdomainAndNotCDNIPs = []  # 子域名和非CDN、非云的IP

    for subdomain in notCDNSubdomains:
        # 在run_cSubnet函数的循环中添加延迟
        for ip in SubdomainAndNotCDNIPs:
            time.sleep(0.5)  # 每查询一个IP间隔0.5秒
    ...
    ColorPrinter.info(f'[total: {len(web_host_port)}] web_host_port')
...
def banner():
    ...
    banner = r'''
                                    ...
                                    |   |  |  | |  _| | | | | |/ / |/ _ \    / _ \  
                                    |   |  |  | |_| | |_| | |   <| |  __/   / ___ \ 
                                    |___|__|   \____|\__,_|_|_|\_\_|\___|  /_/   \_\
...
def is_cloudip(ip):
    # 优先从缓存获取结果
    if ip in cloudip_cache:
        return cloudip_cache[ip]
    ...
    try:
        ...
        if not data_list:
            ...
            result = '云' in location  # 检查是否包含“云”

        cloudip_cache[ip] = result  # 缓存结果
        return result

    except (requests.RequestException, json.JSONDecodeError, IndexError) as e:
        print(f"IP云查询出错（{ip}）: {str(e)}")
        result = False  # 出错时默认视为非云IP
        cloudip_cache[ip] = result  # 缓存错误结果
        return result
...
def _init():
    ...
    # 外网代理
        ...
        try:
            ...
            print("配置文件 ./iniFile/config.ini 未找到，请检查文件路径。")
            exit()
        except UnicodeDecodeError:
            print("配置文件编码可能不是 UTF-8，请检查文件编码。")
            exit()
