...
class SearchDuckDuckGo:
    ...
    async def do_search(self):
        ...
        self.results = first_resp[0] if first_resp else ""
        self.totalresults += self.results
        urls = await self.crawl(self.results)
        urls = {url for url in urls if len(url) > 5}
        all_resps = await AsyncFetcher.fetch_all(urls, proxy=self.proxy)
        self.totalresults += ''.join(all_resps)

    async def crawl(self, text):
        urls = set()
        try:
            load = json.loads(text) if text else {}
            for keys in load.keys():
                val = load.get(keys)
                if isinstance(val, (int, dict)) or val is None:
                    continue
            ...
            for url in urls:
                if '<' in url and 'href=' in url:
                    equal_index = url.index('=') if '=' in url else -1
                    if equal_index != -1:
                        true_url = ''
