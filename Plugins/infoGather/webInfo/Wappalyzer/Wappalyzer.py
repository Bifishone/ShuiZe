...
class WebPage(object):
    ...
    def _parse_html(self):
        ...
        try:
            ...
            self.meta = {
                meta.get('name', '').lower():
                    meta.get('content', '') for meta in soup.find_all(
    ...
    def new_from_url(cls, url, headers=None, verify=True):
        ...
        try:
            ...
            logger.error(f"请求 {url} 时发生网络错误: {req_err}")
            return None
        except Exception as e:
...
class Wappalyzer(object):
    ...
    def latest(cls, apps_file=None):
        ...
        try:
            if apps_file:
                with open(apps_file, 'r', encoding='utf-8') as fd:
                    obj = json.load(fd)
            else:
            ...
            logger.error(f"未找到应用数据库文件: {apps_file}")
            return None
        except json.JSONDecodeError as json_err:
            logger.error(f"解析应用数据库文件时发生 JSON 错误: {json_err}")
            return None
        except Exception as e:
            logger.error(f"初始化 Wappalyzer 时发生未知错误: {e}")
            return None
    ...
    def _prepare_app(self, app):
        ...
        for key in ['url', 'html', 'script', 'implies']:
            app[key] = app.get(key, [])
            if not isinstance(app[key], list):
                app[key] = [app[key]]
        ...
        if not isinstance(app['meta'], dict):
            app['meta'] = {'generator': app['meta']}
        ...
        for key in ['headers', 'meta']:
            for name, pattern in app[key].items():
                app[key][name] = self._prepare_pattern(pattern)
    ...
    def _prepare_pattern(self, pattern):
        ...
        try:
            ...
            warnings.warn(
                f"Caught '{e}' compiling regex: {regex}"
            )
    ...
    def get_categories(self, app_name):
        ...
        cat_names = [self.categories.get(str(cat_num), "")
